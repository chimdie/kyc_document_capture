...
function isSamsungMultiCameraDevice() {
  ...
  const matchIndex = matchedModelNumber[0].match(/\d{3}/);
  if (matchedModelNumber === null) {
    return false;
  }
  ...
  const modelNumber = parseInt(matchIndex[0], 10);
...
class SmartFileUpload {
  ...
  static getHumanSize(numberOfBytes: number) {
  ...
  static async retrieve(files: FileList) {
    if (files.length > 1) {
...
function scwTemplateString(this: SmartCameraWeb) {
  return `
  ...
  <div id='request-screen' class='flow center'>
    ${
      ...
      <div class="nav back-to-host-nav${
        this.hideBackToHost ? " justify-right" : ""
      }">
        ${
          this.hideBackToHost
            ? ""
  ...
  <div hidden id='id-camera-screen' class='flow center'>
    ...
    <div class='section | flow ${
      this.isPortraitCaptureView ? "portrait" : "landscape"
    }'>
      <div class='id-video-container ${
        ...
        <svg class="image-frame" fill="none" height="259" width="396" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 396 259" ${
          this.isPortraitCaptureView ? "hidden" : ""
        }>
          <use href='#image-frame' />
        </svg>

        <svg class="image-frame-portrait" fill="none" height="527" width="396" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 396 527" ${
          !this.isPortraitCaptureView ? "hidden" : ""
        }>
...
export class SmartCameraWeb extends HTMLElement {
  ...
  requestScreen: HTMLElement | null = null;
  cameraScreen: HTMLElement | null = null;
  reviewScreen: HTMLElement | null = null;
  idEntryScreen: HTMLElement | null = null;
  IDCameraScreen: HTMLElement | null = null;
  IDReviewScreen: HTMLElement | null = null;
  backOfIdEntryScreen: HTMLElement | null = null;
  backOfIDCameraScreen: HTMLElement | null = null;
  backOfIDReviewScreen: HTMLElement | null = null;
  thanksScreen: HTMLElement | null = null;
  videoContainer: HTMLElement | null = null;
  smileCTA: HTMLElement | null = null;
  imageOutline: (SVGElement & SVGGeometryElement) | null = null;
  startImageCapture: HTMLButtonElement | null = null;
  captureIDImage: HTMLElement | null = null;
  captureBackOfIDImage: HTMLElement | null = null;
  reviewImage: HTMLImageElement | null = null;
  IDReviewImage: HTMLImageElement | null = null;
  backOfIDReviewImage: HTMLImageElement | null = null;
  reStartImageCapture: HTMLElement | null = null;
  reCaptureIDImage: HTMLElement | null = null;
  reCaptureBackOfIDImage: HTMLElement | null = null;
  selectSelfie: HTMLElement | null = null;
  selectIDImage: HTMLElement | null = null;
  selectBackOfIDImage: HTMLElement | null = null;
  takeDocumentPhotoButton: HTMLElement | null = null;
  uploadDocumentPhotoButton: HTMLElement | null = null;
  takeBackOfDocumentPhotoButton: HTMLElement | null = null;
  uploadBackOfDocumentPhotoButton: HTMLElement | null = null;
  backToSelfie: HTMLElement | null = null;
  backToIdEntryButton: HTMLElement | null = null;
  backToBackIdEntryButton: HTMLElement | null = null;
  backToIdImageButton: HTMLElement | null = null;

  private _videoStreamDurationInMS: number = 7800;
  private _imageCaptureIntervalInMS: number = 200;
  ...
  private _IDVideo: HTMLVideoElement | null = null;
  ...
  setActiveScreen(element: HTMLElement) {
  ...
  connectedCallback() {
    ...
    if (
      ...
      if (this.shadowRoot?.innerHTML) {
        // TODO: check type and result
        this.shadowRoot.innerHTML = heading.innerHTML;
      }
    }
  }
  ...
  setUpEventListeners() {
    ...
    if (!requestCameraAccessElt) {
      throw new Error("SetupError requesting camera access");
    }

    requestCameraAccessElt.addEventListener("click", () => this.init());
    ...
    if (this.backToBackIdEntryButton) {
      this.backToBackIdEntryButton.addEventListener("click", () => {
        if (!this.backOfIdEntryScreen) return;
    ...
    if (this.selectSelfie) {
      this.selectSelfie.addEventListener("click", () => {
        this._selectSelfie();
      });
    }

    if (this.selectIDImage) {
  ...
  handleSuccess(stream: MediaStream) {
    ...
    const _videoContainer = this.videoContainer?.querySelector("video");
    const videoExists = !!_videoContainer;
  ...
  handleIDStream(stream: MediaStream) {
    ...
    let video = null;
    if (videoExists && this.IDCameraScreen && this.backOfIDCameraScreen) {
    ...
    video.onloadedmetadata = () => {
      const actionsElt =
        videoContainer?.querySelector<HTMLButtonElement>(".actions");

      if (!actionsElt) return;
      actionsElt.hidden = false;
    };

    if (!videoExists && videoContainer) {
  ...
  _startImageCapture() {
    ...
    const imageOutlineLength = this.imageOutline?.getTotalLength() || 0;
    ...
    if (this.imageOutline) {
      ...
      this.imageOutline.style.strokeDashoffset = imageOutlineLength.toString();
    ...
    // if(this._imageCaptureIntervalInMS
    this._imageCaptureInterval = window.setInterval(() => {
      this._capturePOLPhoto();
    }, this._imageCaptureIntervalInMS);

    this._videoStreamTimeout = window.setTimeout(() => {
      if (this._stream) {
  ...
  _captureIDImage() {
    ...
    if (this.activeScreen === this.IDCameraScreen && this.IDReviewImage) {
      this.IDReviewImage.src = image;
    } else if (this.backOfIDReviewImage) {
  ...
  _drawImage(canvas: HTMLCanvasElement, video = this._video) {
    ...
    if (!video) {
      throw new Error("Error video not initialized");
    }
  ...
  async _uploadDocument(event: Event) {
    ...
    try {
      ...
      const fileData = (await SmartFileUpload.retrieve(files)) as string;
      ...
      if (!nextScreen) {
        throw new Error("Could not load next screen");
      }
  ...
  _drawIDImage(video = this._IDVideo) {
    if (!video) {
      throw new Error("Could not load ID video element");
    }
    ...
    if (this.isPortraitCaptureView) {
      ...
      if (!ctx) {
        throw new Error("Could not retrive context");
      }
  ...
  _stopVideoStream(stream: MediaStream) {
    clearTimeout(this._videoStreamTimeout || 0);
    clearInterval(this._imageCaptureInterval || 0);
    clearInterval(this._drawingInterval || 0);
    if (this.smileCTA) {
    ...
    if (this.reviewImage && this._referenceImage) {
      this.reviewImage.src = this._referenceImage;
    }

    const totalNoOfFrames = this._rawImages?.length || 0;

    const livenessFramesIndices = getLivenessFramesIndices(totalNoOfFrames);

    if (this._data && this._rawImages) {
  ...
  async _startIDCamera() {
    try {
      ...
      if (this.activeScreen === this.idEntryScreen && this.IDCameraScreen) {
        this.setActiveScreen(this.IDCameraScreen);
      } else if (
        this.activeScreen === this.backOfIdEntryScreen &&
  ...
  async _reStartImageCapture() {
    if (!(this.startImageCapture && this._data)) {
      throw new Error("Could not re-start image capture");
    }
  ...
  async _reCaptureIDImage() {
    ...
    if (!previousScreen) {
      throw new Error("Could not return to previous screen");
    }
